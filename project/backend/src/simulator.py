...
def _dict_from_pairs(pairs: Tuple[Tuple[str, str], ...]) -> Dict[str, str]:
    return dict(pairs)
...
class State:
    ...
    # --- vistas convenientes ---
    # _dict_from_pairs cachea la conversión tupla->dict: la poda de objetos
    # muertos (prune_dead_ground) consulta doors_map()/panels_map() una vez
    # por ítem del suelo y en la mayoría de los sucesores E no cambia
    # (MOVE/PICKUP/DROP no tocan puertas ni paneles), así que la misma tupla
    # se repite muchísimas veces durante la búsqueda: cachear evita reconstruir
    # el dict cientos de miles de veces con el mismo contenido.
    def doors_map(self) -> Dict[str, str]:
        return _dict_from_pairs(self.doors)

    def panels_map(self) -> Dict[str, str]:
        return _dict_from_pairs(self.panels)
...
@dataclass(frozen=True)
class ActionRecord:
    """Sucesor de Applicable(s). `step` trae el paso ya en el formato del
    contrato, EXCEPTO cuando kind == "SWAP": ahí `step` es
    {"_swap": [drop_step, pickup_step]} porque SWAP es una macro-acción
    interna del agente (DROP+PICKUP fusionados por eficiencia de búsqueda,
    ver "SWAP" en design.md) que el contrato no conoce como operación única
    — build_plan() la traduce a sus dos pasos reales antes de responder."""

    kind: str  # "MOVE" | "PICKUP" | "DROP" | "INTERACT" | "SWAP" (interno, se traduce)
    step: Dict[str, Any]
    cost: int
...
class ScenarioIndex:
    ...
    def initial_state(self) -> State:
        ...
        return prune_dead_ground(self, state)
...
def prune_dead_ground(idx: ScenarioIndex, state: State) -> State:
    """Olvida la posición en el suelo de objetos ya muertos (design.md,
    "Relevancia"). Una llave cuya puerta ya abrió, o una herramienta/material
    que ningún panel DAMAGED pendiente necesita, no puede habilitar ninguna
    acción futura sin importar en qué zona quedó tirada. Sin este olvido,
    cada zona distinta donde un DROP forzado la dejó cuenta como un mundo
    nuevo para CLOSED, y el espacio de estados se dispara con las zonas
    posibles para cada objeto ya inútil (ver "Evidencia empírica" abajo)."""
    new_gk = tuple((k, z) for k, z in state.ground_keys if idx.key_relevant(k, state))
    new_gt = tuple((t, z) for t, z in state.ground_tools if idx.tool_relevant(t, state))
    new_gm = tuple((t, z, q) for t, z, q in state.ground_materials if idx.material_relevant(t, state))
    if new_gk == state.ground_keys and new_gt == state.ground_tools and new_gm == state.ground_materials:
        return state
    return dataclass_replace(state, ground_keys=new_gk, ground_tools=new_gt, ground_materials=new_gm)
...
def get_successors(idx: ScenarioIndex, state: State) -> List[Tuple[ActionRecord, State]]:
    ...
    def mk(zone=None, battery=None, pk=None, pt=None, pm=None, dr=None, pn=None, st=None, gk=None, gt=None, gm=None, prune=False) -> State:
        new_state = State(
        ...
        # `state` (el padre) ya está podado por invariante: initial_state() y
        # todo mk() anterior pasan por aquí. MOVE/RECHARGE/PICKUP/ACTIVATE sólo
        # quitan cosas del suelo o no lo tocan, así que nunca pueden reintroducir
        # un objeto muerto — podar ahí sería trabajo repetido sin efecto.
        # Sólo hace falta volver a podar cuando la transición puede *crear* una
        # entrada muerta nueva: E cambió (OPEN_DOOR/REPAIR) o se acaba de
        # soltar al suelo un objeto que ya era irrelevante (DROP/SWAP). Esos
        # llamados pasan prune=True explícitamente.
        return prune_dead_ground(idx, new_state) if prune else new_state
    ...
    # --- SWAP (DROP+PICKUP fusionados) y DROP suelto de reserva ---
    # Sólo aplica si la capacidad está llena y hay en la zona actual un ítem
    # relevante que el robot necesita recoger pero no le cabe. design.md,
    # "Por qué esta restricción no pierde el plan óptimo": un plan que suelta
    # y recoge por separado se reordena sin cambiar costo, así que fusionar
    # ambos pasos en un solo sucesor no pierde ningún plan óptimo — sólo evita
    # registrar en CLOSED el estado intermedio "recién soltado" como un nodo
    # de búsqueda aparte.
    cost_drop = idx.action_costs["drop"]
    if weight >= cap and state.battery >= cost_drop:
        blocked = (
            [("key", k) for k, z in ground_keys.items() if z == state.zone and idx.key_relevant(k, state)]
            + [("tool", t) for t, z in ground_tools.items() if z == state.zone and idx.tool_relevant(t, state)]
            + [("material", t) for t, (z, q) in ground_materials.items() if z == state.zone and q > 0 and idx.material_relevant(t, state)]
        )
        if blocked:
            carried = (
            ...
            # Si hay candidatos irrelevantes, son justo los que se sueltan
            # (política "muertos primero"): esa nueva entrada en el suelo hay
            # que podarla de inmediato. Si no hay irrelevantes, lo que se
            # suelta sigue vivo y no hace falta podar nada.
            dropping_dead = candidates is irrelevant
            swap_generated = False
            swap_cost = cost_drop + cost_pickup
            if state.battery >= swap_cost:
                for dkind, dident in candidates:
                    for pkind, pident in blocked:
                        # Aplica DROP(dident) seguido de PICKUP(pident) sobre copias
                        # locales, en un solo paso (sin reconstruir State ni volver
                        # a llamar get_successors — eso es lo que hace rápido a SWAP).
                        pk, pt = list(state.payload_keys), list(state.payload_tools)
                        pm_map = dict(state.payload_materials)
                        gk, gt, gm = dict(ground_keys), dict(ground_tools), dict(ground_materials)

                        if dkind == "key":
                            pk.remove(dident)
                            gk[dident] = state.zone
                        elif dkind == "tool":
                            pt.remove(dident)
                            gt[dident] = state.zone
                        else:
                            pm_map[dident] -= 1
                            if pm_map[dident] <= 0:
                                del pm_map[dident]
                            z0, q0 = gm.get(dident, (state.zone, 0))
                            gm[dident] = (state.zone, q0 + 1) if z0 == state.zone else (state.zone, 1)
                        ...
                        if pkind == "key":
                            ...
                            z1, q1 = gm[pident]
                            pm_map[pident] = pm_map.get(pident, 0) + 1
                            if q1 - 1 <= 0:
                                del gm[pident]
                            else:
                                gm[pident] = (z1, q1 - 1)

                        if len(pk) + len(pt) + sum(pm_map.values()) > cap:
                            continue  # este DROP solo no libera suficiente espacio (pesos > 1)

                        swap_generated = True
                        drop_step = {"op": "DROP", "item": dident, "cost": cost_drop}
                        pickup_step = {"op": "PICKUP", "item": pident, "cost": cost_pickup}
                        out.append((
                            ActionRecord("SWAP", {"_swap": [drop_step, pickup_step]}, swap_cost),
                            mk(
                                battery=state.battery - swap_cost,
                                pk=tuple(sorted(pk)), pt=tuple(sorted(pt)), pm=_c(pm_map.items()),
                                gk=tuple(sorted(gk.items())), gt=tuple(sorted(gt.items())),
                                gm=tuple(sorted((t, z, q) for t, (z, q) in gm.items())),
                                prune=dropping_dead,
                            ),
                        ))

            if not swap_generated:
                # Reserva: ningún DROP individual liberó espacio suficiente
                # (posible con pesos > 1) para recoger algo relevante en un
                # solo intercambio. Se ofrece el DROP suelto para permitir
                # combinaciones de varios DROP antes del PICKUP.
                for kind, ident in candidates:
                    if kind == "key":
                        ...
                        out.append((ActionRecord("DROP", step, cost_drop), mk(battery=state.battery - cost_drop, pk=new_pk, gk=new_gk, prune=dropping_dead)))
                    elif kind == "tool":
                        new_pt = tuple(x for x in state.payload_tools if x != ident)
                        new_gt = tuple(sorted(list(state.ground_tools) + [(ident, state.zone)]))
                        step = {"op": "DROP", "item": ident, "cost": cost_drop}
                        out.append((ActionRecord("DROP", step, cost_drop), mk(battery=state.battery - cost_drop, pt=new_pt, gt=new_gt, prune=dropping_dead)))
                    else:
                        pm_map = dict(state.payload_materials)
                        ...
                        step = {"op": "DROP", "item": ident, "cost": cost_drop}
                        out.append((ActionRecord("DROP", step, cost_drop), mk(battery=state.battery - cost_drop, pm=_c(pm_map.items()), gm=new_gm, prune=dropping_dead)))
    ...
    if state.battery >= cost_interact:
        for door_id, door in idx.doors.items():
            ...
            # prune=True: abrir la puerta puede matar la relevancia de su llave
            # (y de cualquier otra que ya no proteja nada) — hay que re-podar E.
            out.append((ActionRecord("INTERACT", step, cost_interact), mk(battery=state.battery - cost_interact, dr=new_doors, prune=True)))
        ...
        for panel_id, panel in idx.panels.items():
            ...
            # prune=True: reparar el panel puede dejar sin uso a la herramienta
            # o al tipo de material que le tocaba — hay que re-podar E.
            out.append((ActionRecord("INTERACT", step, cost_interact), mk(battery=state.battery - cost_interact, pm=_c(pm_map.items()), pn=new_panels, prune=True)))

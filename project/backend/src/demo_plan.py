...
def build_plan(scenario_path: Optional[str] = None, scenario_raw: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Traducción al contrato: SWAP es una macro-acción interna (ver
    # ActionRecord en simulator.py) que no existe como operación del
    # contrato — se expande a su DROP + su PICKUP reales.
    steps: List[Dict[str, Any]] = []
    for a in result.plan:
        if a.kind == "SWAP":
            steps.extend(a.step["_swap"])
        else:
            steps.append(a.step)

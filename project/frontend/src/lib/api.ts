...
export async function runPlan(): Promise<void> {
  ...
  // try/finally: si applyStep o la animación lanzan una excepción inesperada,
  // el botón EXECUTE PLAN no debe quedar deshabilitado para siempre — RESET
  // ya libera `running`, pero no debería hacer falta pulsarlo.
  try {
    let current = runtime
